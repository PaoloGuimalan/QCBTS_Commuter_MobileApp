export const SET_AUTH_DETAILS = 'SET_AUTH_DETAILS';
export const SET_PROFILE_DETAILS = 'SET_PROFILE_DETAILS';

export const SET_BUS_STOPS_LIST = 'SET_BUS_STOPS_LIST';
export const SET_ENABLE_LOCATION = 'SET_ENABLE_LOCATION';

export const SET_ROUTES_LIST = 'SET_ROUTES_LIST';
export const SET_SELECTED_ROUTE = 'SET_SELECTED_ROUTE';

export const SET_CURRENT_TAB = 'SET_CURRENT_TAB';

export const SET_CURRENT_LOCATION = 'SET_CURRENT_LOCATION';
export const SET_SELECTED_BUS_STOP = 'SET_SELECTED_BUS_STOP';

export const SET_FEED_LIST = 'SET_FEED_LIST';
export const SET_WAITING_BUS_STOP = 'SET_WAITING_BUS_STOP';

export const SET_FEED_INFO = 'SET_FEED_INFO';

export const SET_LIVE_BUST_LIST = 'SET_LIVE_BUST_LIST';
export const SET_LIVE_ROUTE_LIST = 'SET_LIVE_ROUTE_LIST';

export const SET_SELECTED_LIVE_BUS = 'SET_SELECTED_LIVE_BUS';
export const SET_ASSIGNED_ROUTES_LIST = 'SET_ASSIGNED_ROUTES_LIST';

export const SET_SEARCH_RESULT_LIST = 'SET_SEARCH_RESULT_LIST';

export const SET_INITIAL_MAP_TRIGGER = 'SET_INITIAL_MAP_TRIGGER';

export const SET_ENABLE_LIVEMAP = 'SET_ENABLE_LIVEMAP';
export const SET_ANIMATE_TO_LOCATION_TRIGGER = 'SET_ANIMATE_TO_LOCATION_TRIGGER';